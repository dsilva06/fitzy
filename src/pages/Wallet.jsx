...
export default function WalletPage() {
  ...
  const { data: paymentMethodsData, isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.PaymentMethod.filter({ user_id: user?.id }, "-created_date"),
    enabled: !!user,
    initialData: [],
  });
  ...
  const defaultMutation = useMutation({
    mutationFn: async (idToSetDefault) => {
      ...
      if (currentDefault && currentDefault.id !== idToSetDefault) {
        await base44.entities.PaymentMethod.update(currentDefault.id, { is_default: false });
      }
      await base44.entities.PaymentMethod.update(idToSetDefault, { is_default: true });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['paymentMethods'] });
    }
