...
export default function VenueCard({ venue, selectedDate, onSessionSelect, onWaitlistClick, isInitiallyExpanded = true }) {
    ...
    const { data: sessions, isLoading: sessionsLoading } = useQuery({
        ...
        queryFn: async () => {
            const allSessions = await base44.entities.Session.filter({ venue_id: venue.id }, "start_datetime");
    ...
    const { data: favorite } = useQuery({
        ...
        queryFn: async () => {
            if (!user) return null;
            const favorites = await base44.entities.Favorite.filter({ user_id: user.id, venue_id: venue.id });
            return favorites[0];
        },
        enabled: !!user,
    });
    ...
    const favoriteMutation = useMutation({
        mutationFn: async () => {
            if (favorite) {
                ...
                await base44.entities.Favorite.create({ user_id: user.id, venue_id: venue.id });
            }
        },
        onSuccess: () => {
