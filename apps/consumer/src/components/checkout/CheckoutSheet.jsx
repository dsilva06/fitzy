...
export default function CheckoutSheet({ session, venue, classType, onClose, onSuccess }) {
  ...
  const { data: packageOwnerships } = useQuery({
    ...
    queryFn: () => fitzy.entities.PackageOwnership.filter({ user_id: user.id, status: "active" }),
    enabled: !!user,
    initialData: [],
  });
  ...
  const { data: paymentMethods } = useQuery({
    ...
    queryFn: () => fitzy.entities.PaymentMethod.filter({ user_id: user.id }),
    enabled: !!user,
    initialData: [],
  });
  ...
  const confirmBookingMutation = useMutation({
    mutationFn: async () => {
      // Create booking
      const booking = await fitzy.entities.Booking.create({
        user_id: user.id,
      ...
      if (selectedPaymentMethod?.type === "credits") {
        ...
        for (const ownership of sortedOwnerships) {
          ...
          await fitzy.entities.PackageOwnership.update(ownership.id, {
        ...
        await fitzy.entities.Payment.create({
        ...
        await fitzy.entities.Booking.update(booking.id, { status: "confirmed" });
      } else {
        // Card payment
        await fitzy.entities.Payment.create({
          booking_id: booking.id,
